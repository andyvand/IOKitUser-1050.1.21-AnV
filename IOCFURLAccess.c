...
CFTypeRef IOURLCreatePropertyFromResource(CFAllocatorRef alloc, CFURLRef url, CFStringRef property, SInt32 *errorCode)
{
    CFErrorRef error;
    CFTypeRef retval;
    CFURLCopyResourcePropertyForKey(url, property, &retval, &error);
...
Boolean IOURLCreateDataAndPropertiesFromResource(CFAllocatorRef alloc, CFURLRef url, CFDataRef *resourceData, CFDictionaryRef * properties, CFArrayRef desiredProperties, SInt32 *errorCode)
{
    ...
    if (properties != NULL)
    {
        *properties = CFURLCopyResourcePropertiesForKeys(url, desiredProperties, &error);
        propertiesSet = TRUE;
    }
    ...
    if (errorCode != NULL)
    {
        if (propertiesSet == TRUE)
        {
            ...
            *errorCode = kCFNotFound;

            return FALSE;
        }
    }

    return (CFErrorGetCode(error) == 0);
    /*return (CFURLCreateDataAndPropertiesFromResource(alloc, url, resourceData, properties, desiredProperties, errorCode));*/
}

Boolean IOCFURLWriteDataAndPropertiesToResource(CFURLRef url, CFDataRef __unused dataToWrite, CFDictionaryRef propertiesToWrite, SInt32 *errorCode)
{
    CFErrorRef error;
    CFURLSetResourcePropertiesForKeys(url, propertiesToWrite, &error);
