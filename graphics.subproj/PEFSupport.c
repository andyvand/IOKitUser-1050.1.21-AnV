...
void IOLoadPEFsFromURL( CFURLRef ndrvDirURL, io_service_t service __unused )
{
    ...
    CFErrorRef  errorRef;
    urlEnum = CFURLEnumeratorCreateForDirectoryURL(kCFAllocatorDefault, ndrvDirURL, kCFURLEnumeratorDefaultBehavior, NULL);
    ...
    while (urlEnum != NULL)
    {
        ...
        urlEnum = CFURLEnumeratorGetNextURL(urlEnum, &ndrvURL, &errorRef);
        ...
        CFURLCopyResourcePropertyForKey(ndrvURL, kCFURLFileSecurityKey, &num, &errorRef);
        /*num = (CFNumberRef)CFURLCreatePropertyFromResource(kCFAllocatorDefault, ndrvURL, kCFURLFilePOSIXMode, &error);*/
        if (!num)
        {
            continue;
        }
    ...
    if (/*ndrvDirContents*/urlEnum)
    {
        CFRelease(/*ndrvDirContents*/urlEnum);
    }
}
