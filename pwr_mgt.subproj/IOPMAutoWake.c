...
#define CALOPTS (kCFCalendarUnitSecond | kCFCalendarUnitMinute | kCFCalendarUnitHour | kCFCalendarUnitDay | kCFCalendarUnitMonth | kCFCalendarUnitYear)
...
static IOReturn doAMaintenanceWake(
    ...
    int                         type)
 {
    ...
    absTime = CFDateGetAbsoluteTime(earliestRequest);
    CFCalendarDecomposeAbsoluteTime(calendar, absTime, CALOPTS);
    CFCalendarSetTimeZone(CALOPTS, myTimeZone);
    CFRelease(myTimeZone);

    bzero(&pmMaintenanceDate, sizeof(pmMaintenanceDate));
    // Conversion into PM Calendar struct
    pmMaintenanceDate.year      = (UInt32)ucal_get(calendar->_cal, UCAL_YEAR, &uerr);
    pmMaintenanceDate.month     = (UInt8)ucal_get(calendar->_cal, UCAL_MONTH, &uerr);
    pmMaintenanceDate.day       = (UInt8)ucal_get(calendar->_cal, UCAL_DAY_OF_WEEK, &uerr);
    pmMaintenanceDate.hour      = (UInt8)ucal_get(calendar->_cal, UCAL_HOUR, &uerr);
    pmMaintenanceDate.minute    = (UInt8)ucal_get(calendar->_cal, UCAL_MINUTE, &uerr);
    pmMaintenanceDate.second    = (UInt8)ucal_get(calendar->_cal, UCAL_SECOND, &uerr);
    
    // Stuff into PM Calendar struct (old)
    ...
    CFDateRef power_date)
{
    ...
    if(!power_date) {
        ...
        CFCalendarDecomposeAbsoluteTime(calendar, wake_time, CALOPTS);
        CFCalendarSetTimeZone(calendar, gmt_tz);
        /*gmt_calendar = CFAbsoluteTimeGetGregorianDate(wake_time, gmt_tz);*/
        CFRelease(gmt_tz);

        cal_date->second    = (UInt8)lround(ucal_get(calendar->_cal, UCAL_SECOND, &uerr));
        if (60 == cal_date->second)
            cal_date->second = 59;
        cal_date->minute    = (UInt8)ucal_get(calendar->_cal, UCAL_MINUTE, &uerr);
        cal_date->hour      = (UInt8)ucal_get(calendar->_cal, UCAL_HOUR, &uerr);
        cal_date->day       = (UInt8)ucal_get(calendar->_cal, UCAL_DAY_OF_WEEK, &uerr);
        cal_date->month     = (UInt8)ucal_get(calendar->_cal, UCAL_MONTH, &uerr);
        cal_date->year      = (UInt32)ucal_get(calendar->_cal, UCAL_YEAR, &uerr);
