...
typedef enum kxld_allocate_flags {
    kKxldAllocateDefault = 0x0,
    kKxldAllocateWritable = 0x1,        /* kxld may write into the allocated memory */
} KXLDAllocateFlags;

typedef void (*KXLDLoggingCallback) (KXLDLogSubsystem sys, KXLDLogLevel level, const char *format, va_list ap, void *user_data);
typedef kxld_addr_t (*KXLDAllocateCallback)(size_t size, KXLDAllocateFlags *flags, void *user_data);
...
typedef struct kxld_array_pool {
...
typedef struct kxld_srcversion {
    uint64_t    version;
    boolean_t   has_srcversion;
} KXLDsrcversion;

typedef enum kxld_versionmin_platforms {
    kKxldVersionMinMacOSX,
    kKxldVersionMiniPhoneOS
} KXLDVersionMinPlatforms;

typedef struct kxld_versionmin {
    KXLDVersionMinPlatforms platform;
    uint32_t version;
    boolean_t has_versionmin;
} KXLDversionmin;

typedef struct kxld_symtab {
    KXLDArray syms;
    KXLDDict cxx_index;
    KXLDDict name_index;
    char *strings;
...
/*********************************************************************
 *********************************************************************/
CFDataRef CFGetDataFromFileURL(CFAllocatorRef allocator, CFURLRef anURL)
{
    ...
    stream = CFReadStreamCreateWithFile(allocator, anURL);
    ...
    dataBuffer = (UInt8 *)CFReadStreamGetBuffer(stream, (CFIndex)(-1), &bytesRead);
    ...
    fileData = CFDataCreate(allocator, dataBuffer, bytesRead);
    ...
    Boolean        createPluginsFlag)
{
    ...
    */
    CFURLCopyResourcePropertyForKey(anURL, CFURLResourceIsReachable(anURL, &errorRef), &dirExists, &errorRef);
    ...
    */
    ...
    urlEnum = CFURLEnumeratorCreateForDirectoryURL(allocator, anURL, kCFURLEnumeratorDefaultBehavior, NULL);
    error = CFErrorGetCode(errorRef);

    if (/*!urlContents*/ !urlEnum || error) {
        /*if (error && (error != kCFURLResourceNotFoundError)) {*/
        if ((error) && (error != kCFNotFound)) {
        ...
        error = kCFURLEnumeratorSuccess;
    }

    /*count = CFArrayGetCount(urlContents);
    for (i = 0; i < count; i++) {
        CFURLRef thisURL = (CFURLRef)CFArrayGetValueAtIndex(urlContents, i);*/
    while ((urlEnum != NULL) && (error == kCFURLEnumeratorSuccess))
    {
        CFURLRef thisURL;

        error = CFURLEnumeratorGetNextURL(urlEnum, &thisURL, &errorRef);
    ...
    CFPropertyListRef         plist)
{
    ...
    */
    ...
    if (format == _kOSKextCacheFormatCFXML ||
        ...
        CFPropertyListWrite(plist, plistStream, kCFPropertyListXMLFormat_v1_0, NULL, NULL);
    ...
    CFPropertyListRef       * cacheContentsOut)
{
    ...
    */
    ...
    cacheData = CFGetDataFromFileURL(kCFAllocatorDefault, cacheFileURL);
    if (cacheData == NULL) {
    /*if (!CFURLCreateDataAndPropertiesFromResource(
        CFGetAllocator(cacheFileURL), cacheFileURL, &cacheData, NULL, NULL, &error)) {*/
    ...
    */
    ...
    if (parseXMLFlag) {
        if (format == _kOSKextCacheFormatCFXML ||
            ...
            cacheContents = CFPropertyListCreateWithData(kCFAllocatorDefault, uncompressedCacheData, kCFPropertyListImmutable, kCFPropertyListXMLFormat_v1_0, &errorRef);
            ...
            cacheContents = IOCFUnserialize((const char *)uncompressedBytes, kCFAllocatorDefault, 0, &errorString);
        } else {
            OSKextLog(/* kext */ NULL,
                kOSKextLogErrorLevel |
                kOSKextLogKextBookkeepingFlag | kOSKextLogFileAccessFlag,
    ...
    CFStringRef resourceType)
{
    ...
    if (!aKext->staticFlags.isFromMkext) {
        ...
        resource = CFGetDataFromFileURL(CFGetAllocator(aKext), resourceURL);
        if (resourceURL == NULL) {
    ...
    CFURLRef  pluginsURL)
{
    ...
    * xxx - should we check for a symlink loop? :-O
    */
    urlEnum = CFURLEnumeratorCreateForDirectoryURL(CFGetAllocator(aKext), anURL, kCFURLEnumeratorDefaultBehavior, NULL);
    ...
    while (urlEnum != NULL) {
        CFURLRef thisURL;
        urlEnum = CFURLEnumeratorGetNextURL(urlEnum, &thisURL, &errorRef);
        /*CFURLRef thisURL = (CFURLRef)CFArrayGetValueAtIndex(urlContents, i);*/
    ...
    CFURLRef  anURL)
{
    ...
    CFArrayRef mkextArray = NULL;
    CFDictionaryRef mkextDict = NULL;
    CFErrorRef error;
    CFStringRef URLString[2] = { kCFURLComponentPath, NULL };
    CFErrorRef errorRef;

    pthread_once(&__sOSKextInitialized, __OSKextInitialize);

    mkextArray = CFArrayCreate(allocator, (const void **)URLString, 1, NULL);
    if ((mkextDict = CFURLCopyResourcePropertiesForKeys(anURL, mkextArray, &error)) == NULL)
    {
        OSKextLogMemError();
        goto finish;
    }

    mkextData = CFGetDataFromFileURL(allocator, anURL);
    if (mkextData == NULL) {
    /*if (!CFURLCreateDataAndPropertiesFromResource(allocator, anURL,
        &mkextData, NULL, NULL, NULL)) {
        OSKextLogMemError();*/
        goto finish;
    }
